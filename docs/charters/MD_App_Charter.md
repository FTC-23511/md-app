...
| 2.0     | 2026-06-15       | Phase 2 revisions recorded per the §9 interface contract. (a) **Photo-storage risk (§4/§8 "Photo storage exceeds free tier") resolved:** the "how" changed from "URLs in DB, photos in Drive, manual upload" to **automatic ingest via a Google service account into the team Shared Drive** — files are Shared-Drive-owned (survive student turnover), the DB stores only a link + thumbnail; YouTube/Vimeo stay native passthrough. Spec: `docs/phase2/04-media.md`. (b) **Discord pulled partially into Phase 2:** a one-way **outbound** Discord webhook posts the daily Software-Change-Log commit digest (`docs/phase2/05-scl-ai.md`). This is post-only and distinct from the **inbound** Discord capture webhook, which remains Phase 3. | _[App Lead]_ |
