...
3. **Resolve `type` to an entry definition.** The frontmatter's `type` field looks up the corresponding `EntryDefinition` from `src/entries/_registry.ts`. If no match: reject with a clear error.
