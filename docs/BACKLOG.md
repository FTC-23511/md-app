...
4. **[forms]** Forms rev2 item 11 — renderer: `EntryForm.tsx` + `FieldRenderer.tsx` + `visibleWhen` evaluator. _Tier: auto-merge._
5. **[forms]** Forms rev2 item 12 — validate + insert helpers: `lib/validate-entry.ts` + `lib/insert-entry.ts`. _Tier: auto-merge._
6. **[forms]** Forms rev2 item 13 — T14 Session Log: `entries/session-log.ts` + `entries/_registry.ts` + `app/(authed)/entries/sessions/new/page.tsx`. _Tier: auto-merge._
7. **[forms]** Forms rev2 item 14 — T15 Outreach Log: `entries/outreach-log.ts` + registry update + `app/(authed)/entries/outreach/new/page.tsx`. _Tier: auto-merge._
8. **[forms]** Forms rev2 item 15 — T16 Meeting Notes: `entries/meeting-notes.ts` + registry update + `app/(authed)/entries/meetings/new/page.tsx`. _Tier: auto-merge._
9. **[forms]** Forms rev2 item 16 — T17 list view: `app/(authed)/entries/list/page.tsx`, `lib/queries.ts` with `listAllEntries()`, `listSummary` exports, type-pill component, empty state, placeholder detail route, dashboard wiring. _Tier: auto-merge._
10. **[config]** Add `.gitattributes` with `* text=auto eol=lf` and `*.bat text eol=crlf` to normalize line endings across platforms. More idiomatic than the Prettier `endOfLine: auto` workaround currently in `.prettierrc.json` (which we can leave as a belt-and-suspenders). Prevents the Windows CRLF/LF cycle that broke `pnpm verify` locally yesterday. _Tier: auto-merge (config only)._

## In progress

<!-- Routine moves items here with the PR link when work starts. -->

- **[forms]** Forms rev2 item 10 — composite blocks part 2 (Story + SpecialtyTriggers). Branch `routine/forms-composite-2`. PR: pending. _Tier: auto-merge._
