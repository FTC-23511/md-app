...
1. **[2B] Contact Log** — `entries/contact-log.ts` + `entries/_registry.ts` + route `app/(authed)/entries/contact/new/page.tsx` (`force-dynamic`) + nav button (list + dashboard) + list pill. Uses only existing blocks (text / single-select `relationship_type`+`relationship_status` / date / long-text) — no new block type. Custom two-table write `lib/insert-contact-log.ts`: insert `contacts` (display_name, role_org, rel-type/status option ids; extras `contact_info`, `how_we_connected`) → insert `contact_logs` (contact_id FK, contact_date, contact_method; extras `topic`, `outcomes_commitments`, `follow_up_next_action`, `follow_up_date`), both `created_via='app'`/`entry_state='complete'`. Per brief Q1 fallback (b): always-create-new contact inline; select-existing/dedup deferred (flag in PR). Detail page resolves contact via `lib/entry-detail.ts` so contact fields render read-only. Auto-merge.
2. **[2B] Hardware Change Log** — `entries/hardware-change-log.ts` + registry + route `app/(authed)/entries/hardware/new/page.tsx` + nav + pill. Typed `subsystem_option_id` (reuse `subsystem`), `change_date`, `version`, `replaces_version`, optional `parent_decision_id` (text); extras `what_changed`, `why`, `deltas` (array of `{metric, was, now}`), `tradeoffs`. Adds one minimal repeating block for `deltas` (per brief Q2 — new block touches `_types.ts` union + `FieldRenderer` + `validate-entry` + `EntryDetailView`). Saves to `hw_change_logs`. Auto-merge.
3. **[2B] Software Change Log (baseline)** — `entries/software-change-log.ts` + registry + route `app/(authed)/entries/software/new/page.tsx` (`defaultEntryState: 'draft'`) + nav + pill. Typed `change_type_option_id` (reuse `change_type`), `change_date`, `commit_hash`, `branch`, optional `parent_decision_id` (text); extras `what_changed`, `why`, `hardware_sensors`, `game_challenge`, `before_behavior`, `after_behavior`, `failure_modes`, `verification`, `files_changed` (array — reuse the repeating block from item 2). AI deep-dive OUT (2G). Saves to `sw_change_logs` with `entry_state='draft'`. Auto-merge.
