...
- **Draft → Complete** support: fill depth fields and outcome fields *after* the initial 5-minute entry (`updateEntry` + edit/fill flow).
...
Phase 2 builds depth-fill and outcome-update on the **current permissive RLS**. The *restrictions* (who can edit what, lock after 24h) are deliberately Phase 3. No conflict — Phase 2 adds the capability, Phase 3 adds the guardrails.
